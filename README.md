...
│   └── selfie_adapter/                  # Bobot Scalar Affine hasil Phase 3
│
├── src/
...
| Target Concept            | Base Pythia (Un-finetuned) Output            | Scalar Affine Adapter (Orthogonal) Output                                      | Transformation Result                          |
| ------------------------- | -------------------------------------------- | ------------------------------------------------------------------------------ | ---------------------------------------------- |
...
This PoC demonstrates that the orthogonal Scalar-Affine architecture has the potential to disentangle empathy and clinical concepts. The immediate next steps involve a two-pronged approach to address our current methodological limitations:

1. **Explainer Resolution Scaling:** We will retrain the Scalar Affine Adapter using a full-scale explanation dataset (10,000+ samples). The goal is to sharpen Pythia-160M’s generation resolution so it can produce specific conceptual descriptions free from pre-training artifacts.
...
# 3. Train the Scalar Affine Adapter
